...
public class RenderBlockOutlineHook {
    ...
    record PartPlacementPreviewRenderer(PartPlacement.Placement placement,
        ...
        public boolean render(BlockOutlineRenderState blockOutlineRenderState,
            ...
            var afterTerrain = blockOutlineRenderState.isTranslucent();
            ...
            renderPart(poseStack, nodes, cameraRelativePos, part, placement.side(), PREVIEW_LINE_WIDTH, false,
                    true, true, afterTerrain);
            renderPart(poseStack, nodes, cameraRelativePos, part, placement.side(), PREVIEW_LINE_WIDTH, false,
                    true, false, afterTerrain);
            return false;
        }
    }
    ...
    record FacadePlacementPreviewRenderer(Direction side, IFacadePart facade,
        ...
        public boolean render(BlockOutlineRenderState blockOutlineRenderState,
            ...
            showFacadePlacementPreview(poseStack, cameraRelativePos, nodes, true, afterTerrain);
            showFacadePlacementPreview(poseStack, cameraRelativePos, nodes, false, afterTerrain);
            return false;
        }
    ...
    record FacadeOutlineRenderer(IFacadePart facade, Direction side,
        ...
        public boolean render(BlockOutlineRenderState blockOutlineRenderState,
            ...
            renderFacade(poseStack, nodes, cameraRelativePos, facade, side, lineWidth, highContrast, false,
                    false, blockOutlineRenderState.isTranslucent());
    ...
    record PartOutlineRenderer(IPart part, Direction side,
        ...
        public boolean render(BlockOutlineRenderState blockOutlineRenderState,
            ...
            renderPart(poseStack, nodes, cameraRelativePos, part, side, lineWidth, highContrast, false, false,
                    blockOutlineRenderState.isTranslucent());
            return true;
        }
    }
    ...
    private static void renderBoxes(PoseStack poseStack,
        ...
        if (preview) {
            ...
            renderBoxes(poseStack, nodes, cameraRelativePos, boxes, renderType, color, lineWidth, afterTerrain);
        } else {
            if (highContrast) {
    ...
    private static void renderBoxes(PoseStack poseStack,
            ...
            float lineWidth,
            boolean afterTerrain) {
        // submitShapeOutline has no offset arguments, so the camera-relative offset that used to be baked into
        // the shape vertices is applied to the pose instead, the way vanilla positions its own block outline.
        poseStack.pushPose();
        poseStack.translate(cameraRelativePos.x, cameraRelativePos.y, cameraRelativePos.z);

        for (var box : boxes) {
            nodes.submitShapeOutline(poseStack, Shapes.create(box), renderType, color, lineWidth, afterTerrain);
        }
