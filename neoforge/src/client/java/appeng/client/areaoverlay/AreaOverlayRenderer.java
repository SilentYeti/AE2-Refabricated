...
public class AreaOverlayRenderer {
    ...
    public void render(IAreaOverlayDataSource area, PoseStack poseStack, SubmitNodeCollector nodes) {
        ...
        // Submitted in the same order the batches used to be flushed in
        submit(nodes, poseStack, AERenderTypes.AREA_OVERLAY_LINE_OCCLUDED, level, allChunks, true, 0x30ffffff);
        submit(nodes, poseStack, AERenderTypes.AREA_OVERLAY_FACE, level, allChunks, false, area.getOverlayColor());
        submit(nodes, poseStack, AERenderTypes.AREA_OVERLAY_LINE, level, allChunks, true, area.getOverlayColor());
    }

    private void submit(SubmitNodeCollector nodes, PoseStack poseStack, RenderType renderType, Level level,
            Collection<ChunkPos> allChunks, boolean renderLines, int color) {
        int[] cols = decomposeColor(color);
        nodes.submitCustomGeometry(poseStack, renderType, (pose, builder) -> {
            for (ChunkPos pos : allChunks) {
                // The submitted pose is fixed, so the per-chunk offset is applied to a copy of it rather than
                // by pushing onto the pose stack as it was before submit-based rendering.
