...
public class SpatialStorageSkyRenderer implements CustomSkyboxRenderer, AutoCloseable {
    ...
    public boolean renderSky(LevelRenderState levelRenderState, SkyRenderState skyRenderState,
        ...
        // The skybox is pitch black and untextured. The six sides only differ by a constant rotation, so the
        // geometry is baked once into a static vertex buffer and the view transform is supplied as a uniform.
        if (skyboxVertices == null) {
            buildSkybox();
        }
        drawQuads("spatial sky skybox", AERenderPipelines.SPATIAL_SKYBOX, skyboxVertices, skyboxQuads,
                writeTransform(modelViewMatrix, 1.0f));
        ...
        drawQuads("spatial sky sparkles", AERenderPipelines.SPATIAL_SKYBOX_SPARKLES, sparklesVertices, sparklesQuads,
                writeTransform(modelViewMatrix, fade));

        return true;
    }

    private static GpuBufferSlice writeTransform(Matrix4fc modelViewMatrix, float brightness) {
        return RenderSystem.getDynamicUniforms()
                .writeTransform(new Matrix4f(modelViewMatrix),
                        new Vector4f(brightness, brightness, brightness, 1.0F), new Vector3f(), new Matrix4f());
    }

    private static void drawQuads(String label, RenderPipeline pipeline, GpuBuffer vertices, int quadCount,
        ...
        // getBuffer takes an index count, not a vertex count: QUADS needs six indices per four vertices.
        var indexCount = PrimitiveTopology.QUADS.indexCount(quadCount * 4);
    ...
    private void buildSkybox() {
        // One quad per side, the side's rotation baked into the vertices
        ...
        try (var bytebufferbuilder = new ByteBufferBuilder(
            ...
            for (Quaternionf rotation : SKYBOX_SIDE_ROTATIONS) {
                for (var corner : corners) {
                    var v = rotation.transform(new Vector3f(corner));
                    vb.addVertex(v.x, v.y, v.z).setColor(0f, 0f, 0f, 1f);
                }
                skyboxQuads++;
            }
