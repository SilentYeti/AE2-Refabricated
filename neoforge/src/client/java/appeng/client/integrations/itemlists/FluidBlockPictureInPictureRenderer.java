...
public class FluidBlockPictureInPictureRenderer
    ...
    protected void renderToTexture(State renderState, PoseStack poseStack, SubmitNodeCollector nodes) {
        ...
        // A submitted node carries a single render type, so each of the two sheets the fluid can land in gets its
        // own submit; the tesselation discards whatever does not belong to the sheet being submitted.
        submitSheet(nodes, poseStack, fluidRenderer, fluidState, false);
        submitSheet(nodes, poseStack, fluidRenderer, fluidState, true);

        poseStack.popPose();
    }

    private static void submitSheet(SubmitNodeCollector nodes, PoseStack poseStack, FluidRenderer fluidRenderer,
            FluidState fluidState, boolean translucent) {
        var sheet = translucent ? Sheets.translucentBlockItemSheet() : Sheets.cutoutBlockItemSheet();
        nodes.submitCustomGeometry(poseStack, sheet, (pose, buffer) -> {
            var wrapped = new LiquidVertexConsumer(buffer, pose);
            fluidRenderer.tesselate(
                    BlockAndTintGetter.EMPTY,
                    BlockPos.ZERO,
                    layer -> layer.translucent() == translucent ? wrapped : DiscardingVertexConsumer.INSTANCE,
    ...
    /**
     * Swallows everything the {@link FluidRenderer} emits into the sheet that is not currently being submitted.
     */
